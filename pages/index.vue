...
function getRealTimeStatistic() {
  ...
  onValue(
    query(
      ...
      limitToLast(statisticsLength + 1)
    ),
    (snapshot) => {
      data = []
      snapshot.forEach((v) => {
      ...
      const unixTime = data.at(-1)?.time ?? moment().unix()
      ...
      for (let i = 0; i <= statisticsLength; i++) {
        if (data[i] != undefined) {
          ...
          const clockTime = unixTime - (statisticsLength + i)
          statistics.unshift({
            "vpln": 0,
            "vplts": 0,
            "vbatt": 0.0,
            ...
            "lplts": 0,
            "relay": {
              "n1": { "source": "plts", "load": 0 },
              "n2": { "source": "plts", "load": 0 },
              "n3": { "source": "plts", "load": 0 }
            },
            "time": clockTime
          })
        }
...
<template>
  <div class="mx-auto grid w-full max-w-7xl grid-flow-row gap-y-2.5 px-3 py-3">
    <div
      v-if="statisticsDataLength != 0 && statisticsData.at(-1)?.time! + 3 <= unixTime"
